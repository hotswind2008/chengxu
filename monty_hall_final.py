...
class MontyHallGame:
    ...
    def setup_ui(self):
        ...
        subtitle = tk.Label(
            title_frame,
            text="Monty Hall Problem - 换门还是不换？真实emoji显示！",
            font=('Arial', 12, 'bold'),
            bg='#2c3e50',
            fg='#ffffff'
        )
    ...
    def new_game(self):
        ...
        for i in range(3):
            self.door_content_labels[i].config(text="🚪", font=('Arial', 130), bg='#8b4513', fg='#FFD700')
            self.door_frames[i].config(bg='#8b4513', relief='raised')
            self.door_labels[i].config(text=f"门 {i+1}", fg='#000000', font=('Arial', 22, 'bold'))
