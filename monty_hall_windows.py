...
# 配置matplotlib
plt.rcParams['font.sans-serif'] = ['Microsoft YaHei', 'SimHei', 'Arial Unicode MS', 'STHeiti', 'PingFang SC']
...
class MontyHallGame:
    def __init__(self, root):
        ...
        if self.os_type == 'Windows':
            self.emoji_size_door = 110  # Windows emoji稍小
            self.emoji_size_prize = 130
        else:
            self.emoji_size_door = 130
            self.emoji_size_prize = 150
    ...
    def setup_ui(self):
        ...
        subtitle = tk.Label(
            title_frame,
            text=f"Monty Hall Problem - {system_info}",
            font=('Arial', 11, 'bold'),
            bg='#2c3e50',
            fg='#ffffff'
        )
        ...
        self.info_label = tk.Label(
            left_frame,
            text="请选择一扇门，猜猜汽车在哪里？",
            font=('Microsoft YaHei', 16, 'bold') if self.os_type == 'Windows' else ('Arial', 16, 'bold'),
            fg='#c0392b',
            bg='#ecf0f1',
            wraplength=700,
            justify='center',
        ...
        for i in range(3):
            ...
            content_label = tk.Label(
                door_frame,
                text="🚪",
                font=('Segoe UI Emoji', self.emoji_size_door) if self.os_type == 'Windows' 
                     else ('Arial', self.emoji_size_door),
                bg='#8b4513',
                fg='#FFD700',
                cursor='hand2'
            )
            content_label.place(relx=0.5, rely=0.5, anchor='center')
            content_label.bind('<Button-1>', lambda e, door=i: self.door_clicked(door))
            content_label.bind('<Enter>', lambda e, door=i: self.on_hover(door, True))
            content_label.bind('<Leave>', lambda e, door=i: self.on_hover(door, False))
            self.door_content_labels.append(content_label)
            
            # 门号标签
            label = tk.Label(
                container,
                text=f"门 {i+1}",
                font=('Microsoft YaHei', 20, 'bold') if self.os_type == 'Windows' 
                     else ('Arial', 22, 'bold'),
                fg='#000000',
                bg='#ecf0f1'
            )
        ...
        button_font = ('Microsoft YaHei', 13, 'bold') if self.os_type == 'Windows' else ('Arial', 14, 'bold')
        button_style = {
            'font': button_font,
            'padx': 28,
            'pady': 16,
        ...
        stats_font = ('Microsoft YaHei', 16, 'bold') if self.os_type == 'Windows' else ('Arial', 18, 'bold')
        tk.Label(
            right_frame,
            text="📊 实时统计分析",
            font=stats_font,
        ...
        self.stats_label = tk.Label(
            right_frame,
            text="",
            font=('Microsoft YaHei', 12, 'bold') if self.os_type == 'Windows' else ('Arial', 13, 'bold'),
            bg='white',
            fg='#000000',
            justify='center'
        )
        ...
        tk.Label(
            stay_frame,
            text="坚持:",
            font=('Microsoft YaHei', 11, 'bold') if self.os_type == 'Windows' else ('Arial', 12, 'bold'),
            bg='white',
            fg='#c0392b',
            width=6
        ).pack(side='left')
    ...
    def new_game(self):
        ...
        emoji_font = ('Segoe UI Emoji', self.emoji_size_door) if self.os_type == 'Windows' \
                     else ('Arial', self.emoji_size_door)
        
        for i in range(3):
            self.door_content_labels[i].config(text="🚪", font=emoji_font, bg='#8b4513', fg='#FFD700')
    ...
    def open_goat_door(self):
        ...
        emoji_font = ('Segoe UI Emoji', self.emoji_size_prize) if self.os_type == 'Windows' \
                     else ('Arial', self.emoji_size_prize)
        
        self.door_content_labels[self.opened_door].config(
            text="🐐",
            font=emoji_font,
            bg='#95a5a6',
            fg='#000000'
        ...
        self.door_labels[self.opened_door].config(
    ...
    def show_result(self, final, switched, won):
        ...
        for i in range(3):
            if i == self.car_door:
                self.door_content_labels[i].config(
                    text="🚗",
                    font=emoji_font,
...
这个反直觉的结果正是三门问题的精髓所在！
        ...
        rule_font = ('Microsoft YaHei', 18, 'bold') if self.os_type == 'Windows' else ('Arial', 20, 'bold')
        tk.Label(
            w,
            text="📖 三门问题 - 游戏规则",
            font=rule_font,
        ...
        text_font = ('Microsoft YaHei', 12) if self.os_type == 'Windows' else ('Arial', 13)
        txt = tk.Text(
            w,
            font=text_font,
            wrap='word',
        ...
        button_font = ('Microsoft YaHei', 14, 'bold') if self.os_type == 'Windows' else ('Arial', 15, 'bold')
        tk.Button(
            w,
            text="关闭",
            font=button_font,
